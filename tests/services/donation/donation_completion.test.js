...
describe('donation completion', () => {
    ...
    test('invalid phone number', async () => {
        try {
            await handleRequestToCompleteDonation(DONOR_ONE_PHONE_NUMBER, 123456)
            throw 'handleRequestToCompleteDonation did not throw anything' 
        } catch (error) {
            expect(error).toBe(donationCompletionEnums.DONOR_NOT_FOUND)
        }
    })
    ...
    test('valid donor but not assigned to a patient', async () => {
        ...
        try {
            ...
            expect(error).toBe(donationCompletionEnums.NO_PENDING_DONATION_REQUEST_FOUND)
        }
    })
    ...
    test('valid donor and assigned to patient and no verification code supplied', async () => {
        ...
        try {
            await handleRequestToCompleteDonation(DONOR_ONE_PHONE_NUMBER, donor.verificationCode)
            throw 'handleRequestToCompleteDonation did not throw anything' 
        } catch (error) {
            expect(error).toBe(donationCompletionEnums.INCORRECT_VERIFICATION_CODE_ANOTHER_ONE_SENT)
        }
    })
    ...
    test('valid donor and assigned to patient and incorrect verification code supplied', async () => {
        ...
        try {
            await handleRequestToCompleteDonation(DONOR_ONE_PHONE_NUMBER, donor.verificationCode + 1)
            throw 'handleRequestToCompleteDonation did not throw anything' 
        } catch (error) {
            expect(error).toBe(donationCompletionEnums.INCORRECT_VERIFICATION_CODE_ANOTHER_ONE_SENT)
        }
    ...
    test('run out of chances', async () => {
        ...
        try {
            ...
            expect(error).toBe(donationCompletionEnums.INCORRECT_VERIFICATION_CODE_AND_RAN_OUT_OF_CHANCES)
        }
        ...
        try {
            ...
            expect(error).toBe(donationCompletionEnums.RAN_OUT_OF_CHANCES)
        }
    })
})
